...
public abstract class IntervalThread implements Runnable {
	...
	/**
	 * Construct a new interval thread that will run on the given interval with the
	 * given name.
	 * 
	...
	public void start() throws Exception {
		...
		debugLogger.debug("Starting Interval Thread " + Thread.currentThread().getName());
		active = true;

		// If we don't have a thread yet, make one and start it.
		if (runner == null && interval > 0) {
			debugLogger.debug("Thread Not running " + Thread.currentThread().getName());
	...
	public void stop() throws Exception {
		debugLogger.debug("Stopping Interval Thread " + Thread.currentThread().getName());
		active = false;
	}

	/**
	 * Not for public use. This thread is automatically started when a new instance
	 * is retrieved with the getInstance method. Use the start() and stop() methods
	...
	public void run() {
		...
		debugLogger.debug("Running Interval Thread   " + Thread.currentThread().getName());
		...
		while (active) {
			try {
				...
				errorLogger.error("LOC003:- Interval Thread interrupting " + e.getMessage());
				// Ignore
			}
		}
	}
